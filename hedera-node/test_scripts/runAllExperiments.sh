...
runExperiment () 
{
  ...
  rm -rf data/fs
  #cp data/config/application-prod.properties data/config/application.properties
  ...
  if [[ -n $STREAM_SERVER ]] ; then
    ...
    for i in ${!privateAddresses[@]}; do
      if [[ -n $MULTI_NODES_ON_SINGLE_AWS ]] ; then
        ...
        ssh -o StrictHostKeyChecking=no -i $pemfile "$sshUsername@${publicAddresses[$i]}" "cd remoteExperiment; . _hugepage.sh; . _config.sh; . _functions.sh; timeout -k $killAfter $((experimentDuration + 30))s nohup java $MIRROR_JVM_OPTS $EXTRA_JVM_OPTS -jar swirlds.jar >output.log 2>&1 &" &
      else
        ssh -o StrictHostKeyChecking=no -i $pemfile "$sshUsername@${publicAddresses[$i]}" "cd remoteExperiment; . _hugepage.sh; . _config.sh; . _functions.sh; timeout -k $killAfter $((experimentDuration + 30))s nohup java $MIRROR_JVM_OPTS $EXTRA_JVM_OPTS -Dflag=1 -cp swirlds.jar:data/lib/* com.swirlds.platform.Browser -local $i >output.log 2>&1 &" &
      fi
    done   
  fi
