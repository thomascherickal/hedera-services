...
/**
 * Parent class of {@link EventStreamValidator} and {@link RecordStreamValidator}
 *
 * the files for validation are generated by SSHService.makeSha1sumOfStreamedEvents(int, StreamType)}
 */
public abstract class StreamingServerValidator extends Validator {
	...
	private boolean isAnySha1SumMissing(final List<String> sha1sumList) {
		...
		for (int i = 0; i < sha1sumList.size(); i++) {
			...
			if (sha1sum == null || sha1sum.length() == 0) {
				/* were there stream files, but the sumsha1 failed to execute for some reason? */
	...
	/*
		...
		mismatch. For that reason we check from the second to last common last stream file back. If a difference is
		found
		then an error existing in the streaming data. There is a small statistical chance the lastCommonStream is not
		mismatched, but that a new stream file was started after the last line of the leastCommonEvt was written.
		This is
	...
	private boolean isMismatchOnlyLastTwoEvent(final int node) {
		...
		/* Grab all the stream files, and start checking from the stream fle BEFORE the lastCommonStream, if there are
		 any
