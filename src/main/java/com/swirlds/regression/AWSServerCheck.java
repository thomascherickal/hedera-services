...
public class AWSServerCheck {
     ...
     *
     * @param ec2    - Amazon ec2 class to allow getting information from specific region
     * @param region - the short and long name strings of the region for use by ec2 and pricing API object
     */
    private void GetReservationsInRegion(AmazonEC2 ec2, Pair<String, String> region) {
        ...
        /*
        Reservation details returns information on running and stopped instances, but we do not check on the price of
        stopped instances, only running. Therefore it is possible to have no running instances in a region. Instead
        fo reporting the region is at $0.00 we instead clear out the StringBuilder, which will not be added to the
        slack message further down the line.
        */
        if (totalForRegion != 0) {
            currentRegionSlackMsg.insert(currentRegionSlackMsg.indexOf("\n"), "\nTotal Instances: " + totalInstances);
            String regionTotal = String.format("\t\tTOTAL:\t *$%s*", formatCurrency(totalForRegion));
            currentRegionSlackMsg.insert(currentRegionSlackMsg.indexOf("\n"), regionTotal);
        } else {
            currentRegionSlackMsg = new StringBuilder();
        }
	...
	/**
	 * Ensures consistent formatting of currency in all messages
	 * @param currency - amount to be formated
	 * @return
	 */
	private String formatCurrency(double currency) {
		/* formatter returns ".00" if currency is zero. this returns a more human readable value */
        if (currency == 0) {
            return "0.00";
        }
     ...
     *
     * @param inst   - instance id to look up additional details
     * @param region - the short and long name strings of the region for use by ec2 and pricing API object
        ...
        for (Instance instance : inst) {
            ...
            if (instance.getState().getName().equals(("running")) && !KnownReservedInstances.contains(
                ...
                String instacneDetails = String.format(
                        "%s was launched: %s  \tCurrent cost: $%s \t%s\n",
                        name,
                        instance.getLaunchTime().toString(),
                        formatCurrency(instanceCost), instance.getKeyName());
     ...
     *
     * @param instanceType - String description of instancetype used by amazon pricing objects
     * @param region       - region we are checking the pricing for
     ...
     *
     * @param serv - JSON string describing pricing information for a specific type of instance, will include one on demand
     *             listing
